...
import { authenticateUserBackedClaims, resolveUserBackedClaims, } from '../routes/workers/userBackedClaims';
class FakePreparedStatement {
    sql;
    tables;
    value;
    constructor(sql, tables, value) {
        this.sql = sql;
        this.tables = tables;
        this.value = value;
    }
    ...
    async all() {
        ...
        if (this.sql.includes('PRAGMA table_info')) {
            ...
            return {
                results: Object.keys(firstRow).map((name) => ({ name })),
            };
        }
        return { results: rows };
    }
    async first() {
        ...
        return (filtered[0] ? this.projectRow(filtered[0]) : null);
    }
    extractTableName() {
...
//# sourceMappingURL=userBackedClaims.test.js.map