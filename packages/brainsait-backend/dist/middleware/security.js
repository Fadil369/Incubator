...
export const generateTokens = (user) => {
    ...
    const jwtRefreshSecret = SECURITY_CONFIG.JWT_REFRESH_SECRET;
    ...
    }, jwtRefreshSecret, refreshTokenOptions);
...
export const authenticateToken = async (req, res, next) => {
    try {
        ...
        jwt.verify(token, SECURITY_CONFIG.JWT_SECRET, async (err, decoded) => {
            if (err) {
                logSecurityEvent({
        ...
        userRoles.forEach((userRole) => {
            ...
            const rolePermissions = Array.isArray(userRole.permissions)
                ...
                : typeof userRole.permissions === 'object' && userRole.permissions
                    ? Object.values(userRole.permissions)
                    : [];
 ...
 */
export const saudiComplianceHeaders = (req, res, next) => {
