...
router.post('/register', validateRequest(HealthcareSMESchema), async (req, res) => {
    try {
...
router.post('/onboarding-survey', authenticate, validateRequest(OnboardingSurveySchema), async (req, res) => {
    try {
...
// Update SME profile
router.put('/profile', authenticate, validateRequest(HealthcareSMESchema.partial()), async (req, res) => {
    try {
        const userId = req.user?.id;
...
router.get('/learning-recommendations', authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        const sme = await prisma.healthcareSME.findUnique({
            where: { userId },
