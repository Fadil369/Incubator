...
router.post('/register', authenticate, validateRequest(AIChampionRegistrationSchema), async (req, res) => {
    try {
...
router.post('/mentorship/apply', mentorshipApplyLimiter, authenticate, validateRequest(MentorshipApplicationSchema), async (req, res) => {
    try {
...
router.get('/mentorship/applications', authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Authentication required' });
...
router.put('/mentorship/applications/:applicationId', authenticate, async (req, res) => {
    try {
