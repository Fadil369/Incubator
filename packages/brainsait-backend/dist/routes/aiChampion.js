...
router.post('/apply', authenticate, validateRequest(AIChampionApplicationSchema), async (req, res) => {
    try {
...
router.post('/sessions/schedule', authenticate, validateRequest(MentorshipSessionSchema), async (req, res) => {
    try {
...
router.post('/sessions/:sessionId/feedback', authenticate, validateRequest(SessionFeedbackSchema), async (req, res) => {
    try {
    ...
    try {
        ...
        const leaderboard = champions.map(champion => ({
            ...
            stats: {
                ...
                totalHours: champion.mentorships.reduce((total, mentorship) => total + mentorship.sessions.reduce((sum, session) => sum + session.duration, 0), 0) / 60
            }
        })).sort((a, b) => {
...
router.get('/find-mentors', authenticate, async (req, res) => {
    try {
        ...
        const availableMentors = mentors.filter(mentor => mentor.mentorshipCapacity && mentor.mentorships.length < mentor.mentorshipCapacity).map(mentor => ({
            id: mentor.id,
