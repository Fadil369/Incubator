...
import { getTokenUserId, isWorkerAdminRole, normalizeOptionalString, normalizeOrganizations, resolveUserBackedClaims, WorkerUserClaimsError, } from './userBackedClaims';
...
github.use('*', async (c, next) => {
    ...
    try {
        ...
        let authClaims = verified.payload;
        const userId = getTokenUserId(authClaims);
        const defaultOrg = normalizeOptionalString(c.env.GITHUB_ORG);
        const needsClaimEnrichment = Boolean(userId &&
...
function getAuthorizedOrgs(claims) {
    return [
        claims.org,
        claims.startupOrg,
...
function getStartupSlugFromClaims(claims) {
...
async function buildGithubErrorResponse(c, response) {
...
async function proxyGithubResponse(c, path, options = {}) {
...
function requireGithubOrgAccess(c, org) {
...
function requireStartupSlugAccess(c, slug) {
    if (!/^[a-z0-9-]+$/.test(slug)) {
...
function requireRepoTenantAccess(c, repoName) {
...
async function writeGithubAutomationAudit(db, type, target, payload) {
    await db.exec(`
  ...
  `);
    await db.prepare('INSERT OR IGNORE INTO github_automations (id, type, target, payload, created_at) VALUES (?, ?, ?, ?, ?)')
...
function getRepoSlugPrefix(slug) {
    return `${slug}-`;
}
function isStartupRepoName(repoName, startupSlug) {
    return repoName === startupSlug || repoName.startsWith(getRepoSlugPrefix(startupSlug));
}
function requireRepoOwnerAccess(c, owner, repoName) {
...
function requireConfiguredTemplateOwner(c, templateOwner) {
...
github.get('/orgs/:org/repos', async (c) => {
    ...
    if (authError)
        return authError;
    const res = await ghFetch(`/orgs/${org}/repos?type=all&per_page=100&sort=updated`, c.env.GITHUB_TOKEN);
    if (!res.ok) {
    ...
    if (!isAdmin(claims) && startupSlug) {
        return c.json(repos.filter((repo) => typeof repo.name === 'string' && isStartupRepoName(repo.name, startupSlug)));
    }
    return c.json(repos);
});
...
github.get('/orgs/:org/templates', async (c) => {
    ...
    if (authError)
        return authError;
    const res = await ghFetch(`/orgs/${org}/repos?type=all&per_page=100`, c.env.GITHUB_TOKEN);
    if (!res.ok) {
        return buildGithubErrorResponse(c, res);
    }
    const repos = await res.json();
...
github.get('/startups/:slug/repos', async (c) => {
    ...
    const repos = await res.json();
    const startupRepos = repos.filter((r) => typeof r.name === 'string' && isStartupRepoName(r.name, slug));
...
github.post('/automation/repos/from-template', async (c) => {
    ...
    if (templateAccessError)
        return templateAccessError;
    const repoScopeError = requireRepoTenantAccess(c, body.newRepoName);
    if (repoScopeError)
        return repoScopeError;
    const org = c.env.GITHUB_ORG;
    const res = await ghFetch(`/repos/${templateOwner}/${templateRepoName}/generate`, c.env.GITHUB_TOKEN, {
        method: 'POST',
        body: JSON.stringify({
            owner: org,
...
github.post('/automation/workflows/dispatch', async (c) => {
    ...
    const res = await ghFetch(`/repos/${body.owner}/${body.repo}/actions/workflows/${body.workflowId}/dispatches`, c.env.GITHUB_TOKEN, {
        method: 'POST',
