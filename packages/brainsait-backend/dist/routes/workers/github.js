...
async function requireGithubOrgAccess(c) {
    ...
    const allowedOrgs = new Set([payload.org, payload.startupOrg, ...(Array.isArray(payload.organizations) ? payload.organizations : [])]
...
github.get('/orgs/:org/projects', async (c) => {
    const { org } = c.req.param();
    const res = await ghFetch(`/orgs/${org}/projects?state=open&per_page=50`, c.env.GITHUB_TOKEN, { headers: { Accept: 'application/vnd.github.inertia-preview+json' } });
    return new Response(res.body, { status: res.status, headers: { 'Content-Type': 'application/json' } });
});
...
github.get('/startups/:slug/repos', async (c) => {
    ...
    const repos = await res.json();
    const startupRepos = repos.filter((r) => typeof r.name === 'string' && r.name.startsWith(slug));
...
github.get('/repos/:owner/:repo/workflows/:workflowId/runs', async (c) => {
    ...
    const res = await ghFetch(`/repos/${owner}/${repo}/actions/workflows/${workflowId}/runs?per_page=20`, c.env.GITHUB_TOKEN);
    return new Response(res.body, { status: res.status, headers: { 'Content-Type': 'application/json' } });
});
github.get('/repos/:owner/:repo/runs', async (c) => {
    const { owner, repo } = c.req.param();
    const res = await ghFetch(`/repos/${owner}/${repo}/actions/runs?per_page=20`, c.env.GITHUB_TOKEN);
    return new Response(res.body, { status: res.status, headers: { 'Content-Type': 'application/json' } });
});
// ── Releases ──────────────────────────────────────────────────────────────────
github.get('/repos/:owner/:repo/releases', async (c) => {
    const { owner, repo } = c.req.param();
    const res = await ghFetch(`/repos/${owner}/${repo}/releases?per_page=10`, c.env.GITHUB_TOKEN);
    return new Response(res.body, { status: res.status, headers: { 'Content-Type': 'application/json' } });
});
// ── Issues ────────────────────────────────────────────────────────────────────
github.get('/repos/:owner/:repo/issues', async (c) => {
    const { owner, repo } = c.req.param();
    const res = await ghFetch(`/repos/${owner}/${repo}/issues?state=open&per_page=30`, c.env.GITHUB_TOKEN);
    return new Response(res.body, { status: res.status, headers: { 'Content-Type': 'application/json' } });
});
// ── Pull Requests ─────────────────────────────────────────────────────────────
github.get('/repos/:owner/:repo/pulls', async (c) => {
    const { owner, repo } = c.req.param();
    const res = await ghFetch(`/repos/${owner}/${repo}/pulls?state=open&per_page=30`, c.env.GITHUB_TOKEN);
    return new Response(res.body, { status: res.status, headers: { 'Content-Type': 'application/json' } });
});
// ── Repo Projects ─────────────────────────────────────────────────────────────
github.get('/repos/:owner/:repo/projects', async (c) => {
    const { owner, repo } = c.req.param();
    const res = await ghFetch(`/repos/${owner}/${repo}/projects?state=open&per_page=20`, c.env.GITHUB_TOKEN, { headers: { Accept: 'application/vnd.github.inertia-preview+json' } });
    return new Response(res.body, { status: res.status, headers: { 'Content-Type': 'application/json' } });
});
...
github.post('/automation/repos/from-template', async (c) => {
    ...
    // Log to D1
    await c.env.DB.prepare('INSERT OR IGNORE INTO github_automations (id, type, target, payload, created_at) VALUES (?, ?, ?, ?, ?)')
