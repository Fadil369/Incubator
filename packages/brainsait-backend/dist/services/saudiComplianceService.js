...
exports.saudiComplianceService = exports.SaudiComplianceService = void 0;
...
class SaudiComplianceService {
    ...
    async createOrUpdateCompliance(smeId, data) {
        try {
            ...
            if (data.crNumber) {
                const crValidation = (0, saudiValidation_1.validateCRNumber)(data.crNumber);
            ...
            if (data.vatNumber) {
                const vatValidation = (0, saudiValidation_1.validateVATNumber)(data.vatNumber);
            ...
            if (data.waslAddress) {
                const addressValidation = (0, saudiValidation_1.validateSaudiAddress)(data.waslAddress);
            ...
            const complianceData = {
                ...
                crStatus: data.crNumber ? client_1.CRStatus.PENDING : undefined,
                monocNumber: data.monocNumber,
                monocStatus: data.monocNumber ? client_1.MOCIStatus.PENDING : undefined,
                vatNumber: data.vatNumber,
                vatRegistrationDate: data.vatRegistrationDate,
                vatStatus: data.vatNumber ? client_1.VATStatus.PENDING_REGISTRATION : client_1.VATStatus.NOT_APPLICABLE,
    ...
    async performComplianceCheck(smeId) {
        ...
        if (compliance.crNumber && compliance.crStatus === client_1.CRStatus.ACTIVE) {
            score += 20;
        }
        else if (compliance.crNumber && compliance.crStatus === client_1.CRStatus.PENDING) {
            score += 10;
            issues.push({
                category: 'HIGH',
                type: 'CR_PENDING',
        ...
        if (compliance.vatStatus === client_1.VATStatus.REGISTERED) {
            score += 15;
        }
        else if (compliance.vatStatus === client_1.VATStatus.EXEMPTED) {
            score += 15; // Exemption is compliant
        }
        else if (compliance.vatStatus === client_1.VATStatus.PENDING_REGISTRATION) {
            score += 8;
            issues.push({
                category: 'HIGH',
                type: 'VAT_PENDING',
    ...
    async validateCRWithGovernment(crNumber, smeId) {
        try {
            ...
            const crValidation = (0, saudiValidation_1.validateCRNumber)(crNumber);
    ...
    async logAPICall(smeId, requestType, status, requestData, errorMessage, apiEndpoint, responseTime) {
        try {
            ...
            logger_1.logger.error('Failed to log API call', { smeId, requestType, error });
        }
    }
    /**
...
exports.SaudiComplianceService = SaudiComplianceService;
exports.saudiComplianceService = new SaudiComplianceService();
