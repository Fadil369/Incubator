...
class DocumentService {
    static templatesPath = path_1.default.join(process.cwd(), 'templates');
    static outputPath = path_1.default.join(process.cwd(), 'uploads', 'generated-documents');
    /**
    ...
    static async generateFeasibilityStudy(data) {
        try {
            ...
            const filePath = path_1.default.join(this.outputPath, fileName);
            ...
            await fs_1.default.promises.writeFile(filePath.replace('.pdf', '.html'), htmlContent);
            logger_1.logger.info(`Feasibility study generated for: ${data.companyName}`, {
                smeId: data.smeId,
                fileName,
            });
            return {
    ...
    static async generateCertificate(data) {
        try {
            ...
            // Save as HTML (in production, convert to PDF)
            await fs_1.default.promises.writeFile(filePath.replace('.pdf', '.html'), htmlContent);
            logger_1.logger.info(`Certificate generated for: ${data.recipientName}`, {
    ...
    static async deleteDocument(filePath) {
        try {
            if (fs_1.default.existsSync(filePath)) {
                ...
                logger_1.logger.info(`Document deleted: ${filePath}`);
            }
        }
        catch (error) {
