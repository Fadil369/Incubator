 ...
 */
export const generateTokenPair = async (userId, email, role) => {
    ...
    const refreshToken = jwt.sign({ sessionId, userId }, config.jwt.secret, { expiresIn: '7d' } // 7 days
    );
    ...
    await redisClient.setEx(sessionKey, 7 * 24 * 60 * 60, // 7 days in seconds
    JSON.stringify(sessionData));
    // Store refresh token mapping
    const refreshKey = `refresh:${sessionId}`;
    await redisClient.setEx(refreshKey, 7 * 24 * 60 * 60, // 7 days in seconds
    refreshToken);
...
export const revokeAllUserSessions = async (userId) => {
    try {
        ...
        // Revoke all sessions
        await Promise.all(sessionsToRevoke.map(sessionId => revokeSession(sessionId)));
