...
class MarketIntelligenceService {
     ...
     */
    async getHealthcareMarketTrends(industry, region) {
        try {
     ...
     */
    async analyzeCompetitors(industry, region, companySize) {
        try {
            const prompt = `
     ...
     */
    async identifyMarketOpportunities(businessProfile) {
        try {
            const prompt = `
     ...
     */
    async getRegulatoryUpdates(relevantAuthorities = ['MOH', 'NPHIES', 'SAMA']) {
        try {
            const prompt = `
     ...
     */
    async getGovernmentIncentives(businessType, region) {
        try {
            const prompt = `
     ...
     */
    async getMarketIntelligenceDashboard(smeProfile) {
        try {
            const [trends, opportunities, regulations, incentives, competitors] = await Promise.all([
                ...
                this.analyzeCompetitors(smeProfile.industry, smeProfile.region, smeProfile.size)
            ]);
            return {
                trends,
                opportunities,
    ...
    parseMarketOpportunities(response) {
        return [
            {
                id: '1',
    ...
    parseRegulatoryUpdates(response) {
        return [
            {
                id: '1',
    ...
    parseGovernmentIncentives(response) {
        return [
            {
                id: '1',
...
//# sourceMappingURL=marketIntelligenceService.js.map