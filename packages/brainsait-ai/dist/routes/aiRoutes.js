...
router.get('/market/trends', async (req, res) => {
    try {
        ...
        const trends = await marketIntelligenceService.getHealthcareMarketTrends(industry, region);
        res.json({
            success: true,
            data: trends
        });
...
router.get('/market/regulations', async (req, res) => {
    try {
        ...
        const authoritiesList = authorities ? authorities.split(',') : undefined;
...
router.get('/market/incentives', async (req, res) => {
    try {
        ...
        const incentives = await marketIntelligenceService.getGovernmentIncentives(businessType, region);
        res.json({
            success: true,
            data: incentives
        });
...
router.post('/documents/validate-compliance', async (req, res) => {
    try {
        ...
        const validation = await documentAIService.validateCompliance(documentContent, documentType, regulations);
        res.json({
            success: true,
            data: validation
        });
...
router.post('/analytics/predictive', async (req, res) => {
    try {
        ...
        const predictions = await aiAnalyticsService.generatePredictiveAnalytics(historicalData, businessContext);
        res.json({
            success: true,
            data: predictions
        });
