...
class DocumentAIService {
   ...
   */
  async optimizeDocument(request: DocumentOptimizationRequest): Promise<DocumentOptimizationResponse> {
    try {
  ...
  async validateCompliance(
    ...
    documentType: string,
    regulations: string[] = ['MOH', 'NPHIES', 'SAMA']
  ): Promise<{
  ...
  private createOptimizationPrompt(request: DocumentOptimizationRequest): string {
    return `
  ...
  private getSystemPromptForDocumentType(documentType: string): string {
    ...
    return prompts[documentType as keyof typeof prompts] || 'You are a business document optimization expert with knowledge of Saudi healthcare regulations.';
  }

  private parseOptimizationResponse(response: string, request: DocumentOptimizationRequest): DocumentOptimizationResponse {
  ...
  private parseAnalysisResponse(response: string): DocumentAnalysisResponse {
    return {
      overallScore: 78,
      strengths: [
  ...
  private parseTemplateResponse(response: string, request: TemplateGenerationRequest): TemplateGenerationResponse {
    return {
      template: {
        sections: [
          {
  ...
  private parseComplianceResponse(response: string, regulations: string[]): any {
    return {
  ...
  private parseSectionContentResponse(response: string, language: string): any {
    return {
