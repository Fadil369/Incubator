import { incubatorChatMessages as seedChatMessages, incubatorChatRooms as seedChatRooms, incubatorEmailAutomations as seedEmailAutomations, } from '../../../packages/brainsait-shared/src/constants/incubator';
const corsHeaders = {
...
async function listChatRooms(env) {
    const storedRooms = await listJsonByPrefix(env.CHANNEL_CONFIG, 'chat-room:');
    const merged = mergeById(seedChatRooms, storedRooms).sort((left, right) => left.name.localeCompare(right.name));
    return Response.json(merged);
}
async function listChatMessages(roomId, env) {
    await ensureNotificationSchema(env);
    const seed = seedChatMessages.filter((message) => message.roomId === roomId);
    const result = await env.DB.prepare(`SELECT id, room_id, sender_id, sender_name, direction, message, created_at, attachment_name, attachment_url
       FROM chat_messages
      WHERE room_id = ?
      ORDER BY created_at ASC`).bind(roomId).all();
...
async function createChatMessage(roomId, request, env) {
    await ensureNotificationSchema(env);
    ...
    await env.DB.prepare(`INSERT INTO chat_messages (id, room_id, sender_id, sender_name, direction, message, created_at, attachment_name, attachment_url)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(created.id, created.roomId, created.senderId, created.senderName, created.direction, created.message, created.createdAt, created.attachmentName || null, created.attachmentUrl || null).run();
...
async function listEmailAutomations(env) {
    await ensureNotificationSchema(env);
    const result = await env.DB.prepare(`SELECT id, name, trigger_event, subject, recipients_json, template_preview, enabled, last_triggered_at, created_at
    ...
    const kvStored = await listJsonByPrefix(env.CHANNEL_CONFIG, 'email-automation:');
    return Response.json(sortByCreatedAtDesc(mergeById(seedEmailAutomations, [...stored, ...kvStored])));
}
async function createEmailAutomation(request, env) {
    await ensureNotificationSchema(env);
    const body = await request.json();
    ...
    const recipients = Array.isArray(body.recipients)
        ? body.recipients.filter((value) => typeof value === 'string' && value.trim().length > 0).map((value) => value.trim())
        : [];
    ...
    await env.DB.prepare(`INSERT INTO email_automations (id, name, trigger_event, subject, recipients_json, template_preview, enabled, last_triggered_at, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(automation.id, automation.name, automation.triggerEvent, automation.subject, JSON.stringify(automation.recipients), automation.templatePreview, automation.enabled ? 1 : 0, automation.lastTriggeredAt || null, automation.createdAt).run();
...
async function triggerEmailAutomation(automationId, request, env) {
    await ensureNotificationSchema(env);
...
async function listNotificationHistory(limit, env) {
    await ensureNotificationSchema(env);
    const dbRows = await env.DB.prepare(`SELECT id, channel, subject, status, created_at, metadata_json
       FROM notification_history
      ORDER BY created_at DESC
      LIMIT ?`).bind(limit).all();
    const dbHistory = dbRows.results.map((row) => ({
        id: String(row.id),
...
async function routeNotification(event, env) {
...
async function sendDiscord(webhookUrl, event, payload) {
...
async function broadcastWS(event, env, roomId = 'global') {
...
async function getAutomationById(id, env) {
    await ensureNotificationSchema(env);
    const dbRow = await env.DB.prepare(`SELECT id, name, trigger_event, subject, recipients_json, template_preview, enabled, last_triggered_at, created_at
       FROM email_automations
    ...
    if (dbRow) {
        return {
            ...
            recipients: JSON.parse(String(dbRow.recipients_json)),
...
async function persistHistory(env, item) {
    await ensureNotificationSchema(env);
    await env.DB.prepare(`INSERT OR REPLACE INTO notification_history (id, channel, subject, status, created_at, metadata_json)
     VALUES (?, ?, ?, ?, ?, ?)`).bind(item.id, item.channel, item.subject, item.status, item.createdAt, item.metadata ? JSON.stringify(item.metadata) : null).run();
    await env.NOTIFICATION_LOG.put(`history:${item.id}`, JSON.stringify(item), { expirationTtl: 604800 });
}
async function listJsonByPrefix(namespace, prefix) {
...
function normalizeHistoryRecord(record) {
    ...
    if (typeof record.type === 'string' && typeof record.status === 'string') {
        return {
            ...
            metadata: record.data && typeof record.data === 'object' ? record.data : undefined,
        };
    }
    return null;
}
